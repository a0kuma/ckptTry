...
def checkpoint(
    ...
    **kwargs
    ...
    """
    ic('this is the (start) EXEC of DEF checkpoint')
    ...
    if use_reentrant:
        ...
        ic('this is before create the gen object')
        gen = _checkpoint_without_reentrant_generator(
            function, preserve, context_fn, determinism_check, debug, early_stop, *args, **kwargs
        )
        ic('this is after gen obj create and before next(gen)',gen)
        # Runs pre-forward logic
        next(gen)
        try:
            ic('this is after next(gen) and before ret = function(..., ...)')
            ret = function(*args, **kwargs)
            ic('this is after ret = function ...', ret)
        ...
        try:
            ic('this is what is known for before post FWD aka next(gen)')
            next(gen)
            ic('this is what is known for after post FWD aka next(gen)')
...
def _checkpoint_without_reentrant_generator(
    ...
    **kwargs
    ...
    """
    ic('this is the OBJ create of _checkpoint_without_reentrant_generator')
    ...
    ic('this is the place before the with _checkpoint_hook ...')
    with _checkpoint_hook(new_frame), forward_context:
        ic('this is the place after the with _checkpoint_hook ...')
        try: 
            ic('this is the place before the yield')
            yield
            ic('this is the place after the yield')
    ...
    ic(new_frame.forward_completed)
